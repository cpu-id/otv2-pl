...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(53, 53, 53, 255), 2.f);

    if (hovered)
        PushStyleColor(ImGuiCol_Text, ImVec4(1.00f, 1.00f, 1.00f, 1.f));
    else
        PushStyleColor(ImGuiCol_Text, ImVec4(1.00f, 1.00f, 1.00f, 0.55f));
    // Render
    if (hovered) {

        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(159, 57, 57, 255), 2.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min + ImVec2(5,4) , bb_frame.Max - ImVec2(7,4), ImColor(255,255,255,10), 10.f, rounding_corners);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min , grab_rect.Max - ImVec2(2,0), ImColor(200,64,64,255), 15.f);
...
bool ImGui::Checkbox(const char* label, bool* v) 
{
    ...
    const ImGuiID id = window->GetID(label); 
    const ImVec2 label_size = CalcTextSize(label, NULL, true); 
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.x + 42.f, label_size.y)); 
    ...
    if (*v) {
        ...
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 16), ImColor(159,57,57,255), 12.f);
        window->DrawList->AddRect(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 16), ImColor(255,255,255,25), 12.f);
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(22, 8), 5, ImColor(255, 255, 255, 255));
    }
    else {

        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 16), ImColor(55, 55, 55, 255), 12.f);
        window->DrawList->AddRect(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 16), ImColor(255, 255, 255, 25), 12.f);
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(8, 8), 5, ImColor(255, 255, 255, 255));

    }
    if (label_size.x > 0.0f)
        RenderText(check_bb.Min + ImVec2(38.f, -0.f), label);
