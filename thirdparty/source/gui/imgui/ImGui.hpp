...
// Using dear imgui via a accessors library is not recommended, because we don't guarantee backward nor forward ABI compatibility (also function call overhead, as dear imgui is a call-heavy API)
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
struct ImDrawListSharedData;        // Data accessors among multiple draw lists (typically owned by parent ImGui context, but you may create one yourself)
...
enum ImGuiConfigFlags_
{
    ...
    // User storage (to allow your back-end/engine to communicate to code that may be accessors between multiple projects. Those flags are not used by core Dear ImGui)
...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to accessors draw data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)
